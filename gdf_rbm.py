  ...
  def sample(
    ...
    t_guidance_end: float = 0.2,
      ...
      for i in range(0, timesteps):
          ...
          if (t_guidance_start < noise_cond) and (noise_cond < t_guidance_end):
            ...
            g_s = torch.autograd.grad(loss_style, x, retain_graph=False)
            g_s = 1 / (torch.linalg.norm(g_s) + 1e-8) * g_s

            loss_content = torch.linalg.norm(org_content_embedding - this_content, ord=2)
            g_c = torch.autograd.grad(loss_content, x, retain_graph=False)
            g_c = 1 / (torch.linalg.norm(g_c) + 1e-8) * g_c

            u = g_s - Lambda * torch.sum(g_s @ g_c) * g_c

            u = u * (torch.linalg.norm(pred) / (torch.linalg.norm(u) + 1e-8))

            epsilon = pred - eta * (1 - alpha_cumprod[i]).sqrt() * u

          x = sampler(x, x0_hat, epsilon, logSNR_range[i], logSNR_range[i+1], **sampler_params)
          altered_vars = yield (x0_hat, x, pred)
